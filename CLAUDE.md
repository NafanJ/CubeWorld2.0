...
│   ├── functions/
│   │   ├── tick/
│   │   │   └── index.ts        # Core simulation engine (~1200 lines, Deno edge function)
│   │   └── reply/
│   │       └── index.ts        # Instant @mention reply handler (~290 lines)
│   └── migrations/
...
└── web/
    ...
    │       ├── ChatLogTab.tsx      # Realtime message feed + @mention autocomplete
    ...
    │       └── DiaryTab.tsx        # Agent private diary entries viewer
    ├── vite.config.ts
...
- Realtime publication (`supabase_realtime`) includes: `messages`, `agents`, `diary_entries`, `relationships`.
...
- `messages`, `diary_entries`, `relationships` use `bigserial` primary keys; all other tables use `uuid`.
...
**Tick function — 6 phases per invocation:**
1. **Movement** — Agents follow pre-computed `persona.path` arrays, or pick new destinations (15% random, 40% toward high-affinity agents, else random avoiding disliked agents). Pathfinding routes through elevator (x=1) when changing floors.
2. **Affinity Updates** — Co-located agents gain +1 affinity via `upsert_affinity()` RPC.
3. **Conversations + Solo Messages** — Rooms with 2+ agents get a 2–3 turn AI conversation between the highest-affinity pair; remaining agents get solo action messages. LLM returns `{"message": "...", "mood_delta": -1|0|1}`.
4. *(Reserved for idle energy recovery)*
5. **Diary Entries** — Staggered ~1 entry per agent per 10 ticks; first-person introspective reflections generated via OpenAI.
6. **Tick Increment** — Calls `increment_tick_count()` RPC.

**Reply function flow:**
1. Receives `{content, mentions: {agents, rooms}}` from frontend.
2. Inserts visitor message (`from_agent: null`).
3. Resolves @mentions: `@everyone` → all active agents (cap 6); `@agent_name` → specific agent; `@room_name` → agents in that room (targeted cap 3).
4. Generates and inserts a reply from each targeted agent via OpenAI.
...
| `messages` | Agent and visitor messages; `from_agent` is null for visitor messages; includes `room_id`, `content`, `mood_tag` |
| `diary_entries` | Private agent reflections; `agent_id`, `text`, `ts`; now displayed in DiaryTab |
